...
Candidates = `## residue` table (`section|id|pattern|line`) — every live row hit by script `HR_*` / `HR_PATTERN_ORDER` or oversized §T `task` / §B `cause` cells (`oversized-cell`), after the same pre-filters as the audit path.
...
Per-hit prune by table `pattern` cell: drop `(∆)` markers; drop inlined `retired YYYY-MM-DD`; drop `pre-amend` / `prior … retired/dropped/superseded` narration; `oversized-cell` → §T body-trim / §B cause one-line trim (surplus → commit-msg body).
