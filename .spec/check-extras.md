...
V22: recipe-step-no-dispatch — slash-cmd dispatch = operator turn only; recipes end @ commit + Next block; exclusions: (1) /sdd:build verify-fail → mid-loop spec BACKPROP; (2) green-path chain default-on per `skills/_fragments/CHAIN.md` — build pass → check cascade same turn; check clean + pending `.` §T → build --next same turn; `--no-chain` disables; dirty check never auto-remedy; (3) github post-spec-commit chain — spec APPLY commit + draft PR → run `/sdd:build --all` sub-agent then bundled `review` sub-agent same session; no operator re-type.
