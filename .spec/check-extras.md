...
V21: write-serialize — SPEC.md + code writes serialize main-thread; reads delegable to read-only sub-agents; exclusion: github post-spec-commit chain runs write-capable `/sdd:build --all` child and bundled `review` sub-agent (scratch writes only, no repo edits; spawn omits capability_mode read-only); child build verification failure halts and reports to parent session without mid-loop interactive prompts or SPEC.md writes (github-workflow invariant) (closes §B.34).

## §V22 recipe-step-no-dispatch

V22: recipe-step-no-dispatch — slash-cmd dispatch = operator turn only; recipes end @ commit + Next block; exclusions: (1) /sdd:build verify-fail → mid-loop spec BACKPROP; (2) green-path chain default-on per `skills/_fragments/CHAIN.md` — build pass → check cascade same turn; check clean + pending `.` §T → build --next same turn; `--no-chain` disables; dirty check never auto-remedy; (3) github post-spec-commit chain — spec APPLY commit + draft PR → run `/sdd:build --all` sub-agent then bundled `review` sub-agent then github READY remainder same session; no operator re-type (closes §B.33).
...
V31: shape-lifecycle — /sdd:shape uses Grok Plan mode (enter_plan_mode → plan.md → exit_plan_mode); primary artifact = session plan, not `designs/`; post-approve ! open GitHub issue (steno plan body; `## Acceptance` from Success criterion; label enhancement or bug or documentation from plan class, default enhancement) then stop; Next leads issue URL + `/sdd:spec github issue N`; same-session `/sdd:spec fold-shape` retains created issue N linkage; legacy `designs/*.md` still folds; optional `--export designs/<slug>.md` on operator ask; not bundled `/design`.
...
V69: github-workflow — passive `skills/github/SKILL.md` (`user-invocable: false`, auto-fire on gh issue/PR ops per sub-skill-flags invariant) governs gh-CLI workflow: issues via `gh issue create` (body problem + `## Acceptance` lines); every worked GitHub issue ! one issue-linked PR; no corresponding GitHub issue → no git branch + no `gh pr create` (work stays current branch; plain git commit still in scope); `/sdd:spec github issue N` engages github: BRANCH `gh issue develop <n> --checkout` (in-place, one branch per session — no dedicated worktree) before spec APPLY commit of SPEC.md on that branch; then `gh pr create --draft` (generic structure; github-facing bodies steno per github-facing-register invariant; no `Closes`/`Fixes`/`Resolves` trailer @ create; no review-at-create); after that spec APPLY commit: run write-capable `/sdd:build --all` sub-agent (no operator wait; child drops github READY; failure halts and reports to parent) then run bundled Grok `review` skill as sub-agent on issue-linked branch vs default base (load-and-run, not slash-dispatch `/review` per recipe-step-no-dispatch invariant) then github READY remainder (parse+apply findings, push, `gh pr ready`); chain runs once — owner github PR recipe; spec After OK = BRANCH + SPEC.md commit + draft PR only (closes §B.32); spec-side cites three-step list not two-step subset (closes §B.33); spawn prompt ! `POST-SPEC-CHILD=1` (closes §B.35); later issue-linked commits `git push` (github PUSH — issue-linked branch w/ open PR only; plain git push w/ no issue/PR still out of scope); after issue-linked code complete (operator-run build, not post-spec child): bundled Grok `review` skill on the issue-linked branch vs default base (load-and-run, not slash-dispatch `/review` per recipe-step-no-dispatch invariant) → parse findings → apply open bug+suggestion (nits listed, apply unless operator declines) → `git push` → `gh pr ready`; recipe pauses (Next: merge when approved); issue-linked task build verifies task-scoped acceptance evidence; open issue acceptance checklist items missing evidence do not block intermediate task status flip; full acceptance verification gates PR readiness and issue close; build commit never carries `Closes`/`Fixes`/`Resolves`; merge = ACCEPTANCE-GATE then add close trailer to PR body then `gh pr merge --squash --delete-branch --subject "<title> (#<issue>)" --body "Closes #<issue>"` (squash subject ! `#<issue>` the linked issue not merely PR; body ! `Closes #<issue>`; GitHub default subject PR title `(#PR)` insufficient; commits squashed, branch deleted; closes §B.36); unmerged close → `gh pr close` + `git branch -D <branch>`, no squash; issue/PR linkage = native `gh issue develop` linked branch; issue close (MERGE add-trailer, or `gh issue close N` after issue-linked work) ! acceptance-gate: load issue N (`gh issue view`), parse `## Acceptance` bullets (`- [ ]`/`- [x]`); each open bullet ! evidence (test name, code path, or CLI probe); miss → BLOCK close trailer / block merge + FAIL table; all pass → ALLOW close + post Acceptance-evidence comment on issue (? flip checks); ALLOW @ MERGE = add trailer then squash; no `## Acceptance` → advisory not silent-verified; consumer-repo no hardcoded owner/repo (parametric-recipe invariant).
