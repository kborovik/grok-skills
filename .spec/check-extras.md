...
V69: github-workflow — passive `skills/github/SKILL.md` (`user-invocable: false`, auto-fire on gh issue/PR ops per sub-skill-flags invariant) governs gh-CLI workflow: issues via `gh issue create` (body problem + `## Acceptance` lines; shape hands payload to github ISSUE); every worked GitHub issue ! one issue-linked PR; no corresponding GitHub issue → no git branch + no `gh pr create` (work stays current branch; plain git commit still in scope); `/sdd:spec github issue N` or fold-shape with issue N engages github: BRANCH `gh issue develop <issue> --checkout` (clean tree; delta not yet written; in-place, one branch per session) then write delta then SPEC.md commit on that branch then `gh pr create --draft` (generic structure; github-facing bodies steno; no `Closes`/`Fixes`/`Resolves` trailer @ create; no review-at-create); after draft PR: run write-capable `/sdd:build §T.<a>,§T.<b>,…` on fold-produced §T ids only (not whole backlog; spawn prompt ! `POST-SPEC-CHILD=1` implies `--no-chain`; child drops READY; failure halts to parent — Next `/sdd:build` and class b/c `/sdd:spec` BACKPROP) then bundled `review` sub-agent then READY remainder (parse+apply findings, push, `gh pr ready`); chain runs once — owner github PR recipe; spec After OK stops at draft PR (closes §B.32, §B.33, §B.35, §B.48, §B.49); later issue-linked commits `git push` (PUSH); operator-run issue-linked code complete → READY once per run not per task (review-apply → push → `gh pr ready`; Next merge when approved — say "merge the PR"); issue-linked task build = task-scoped acceptance evidence only; full acceptance gate = MERGE / issue close only (not PR ready; closes §B.58); build commit never carries close trailer; MERGE = ACCEPTANCE-GATE then add `Closes #<issue>` then `gh pr merge <pr> --squash --delete-branch --subject "<title> (#<issue>)" --body "Closes #<issue>"` (closes §B.36); evidence comment once per issue-linked run (closes §B.52); unmerged CLOSE → `gh pr close <pr>` then `git switch <default-base>` then `git branch -D <branch>` (closes §B.40); placeholders `<issue>` vs `<pr>` never dual `<n>` (closes §B.55); consumer-repo no hardcoded owner/repo (parametric-recipe invariant).
